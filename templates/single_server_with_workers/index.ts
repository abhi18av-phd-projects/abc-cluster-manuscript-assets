...
//   pulumi config set serverCpus 4       [default: 4]  — server VM CPUs
//   pulumi config set serverMemory 8G    [default: 8G] — server VM memory
//   pulumi config set serverDisk 40G     [default: 40G]— server VM disk
//
//   The server defaults are sized for the interactive workbench, which runs
//   HERE rather than on a worker: TLJH manages per-user notebook servers as
//   systemd units on one machine, so the workers stay free for batch jobs and
//   apps. TLJH's conda stack alone is 2-3 GB before any user logs in, and each
//   active user adds memory on top. With enableWorkbench=false the server does
//   nothing but run the Nomad server and 2 vCPU / 2 GB / 10 GB is ample.
...
// The interactive workbench runs on the SERVER (see the note above), so the
// server's defaults depend on whether it is enabled. TLJH's conda stack is
// 2-3 GB before any user logs in; 2 GB / 10 GB would fail to install.
const enableWorkbench = config.getBoolean("enableWorkbench") ?? true;

const serverCpus   = config.getNumber("serverCpus")    ?? (enableWorkbench ? 4 : 2);
const serverMemory = config.get("serverMemory")        ?? (enableWorkbench ? "8G" : "2G");
const serverDisk   = config.get("serverDisk")          ?? (enableWorkbench ? "40G" : "10G");
